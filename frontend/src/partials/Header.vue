  ...
  <header class="absolute w-full z-30">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
      <div class="flex items-center justify-between h-16 md:h-20">
        ...
        <!-- Desktop navigation -->
        <nav class="flex grow ml-14">
          <ul v-if="isAuth" class="flex grow justify-start flex-wrap items-center">
            <li>
              <router-link class="font-medium text-gray-600 decoration-blue-500 decoration-2 underline-offset-2 hover:underline px-3 lg:px-5 py-2 flex items-center transition duration-150 ease-in-out" to="/my_creatures">My Creatures</router-link>
            </li>
          </ul>


          <!-- Desktop sign in links -->
          <ul v-if="!isAuth" class="flex grow justify-end flex-wrap items-center">
            <li>
              <router-link class="font-medium text-gray-600 decoration-blue-500 decoration-2 underline-offset-2 hover:underline px-3 lg:px-5 py-2 flex items-center transition duration-150 ease-in-out" to="/signin">Sign in</router-link>
            </li>
            <li class="ml-3">
              <router-link class="btn-sm text-white bg-blue-500 hover:bg-blue-600 w-full shadow-sm" to="/signup">Join The Community</router-link>
            </li>
          </ul>

          <ul v-else class="flex grow justify-end flex-wrap items-center">
            <li class="ml-3">
              <button @click="logout" class="btn-sm text-white bg-blue-500 hover:bg-blue-600 w-full shadow-sm" to="/signup">Logout</button>
            </li>
          </ul>
